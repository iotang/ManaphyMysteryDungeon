...
static LONG FAR PASCAL GraphicsEventProc(HWND w, UINT msg, WPARAM p1,
                                         LPARAM p2);
...
static void DisplayArc(double xc, double yc, double rx, double ry, double start,
                       double sweep);
static void RenderArc(double x, double y, double rx, double ry, double start,
                      double sweep);
...
static void SetArcBB(RECT *rp, double xc, double yc, double rx, double ry,
...
string GetWindowTitle(void) { return (CopyString(windowTitle)); }
...
static void InitDisplay(void) {
  ...
  graphicsWindow = CreateWindow(
      GWClassName, windowTitle, style, graphicsRect.left, graphicsRect.top,
      RectWidth(&graphicsRect), RectHeight(&graphicsRect),
      /*consoleWindow*/ NULL, (HMENU)NULL, (HINSTANCE)NULL, (LPSTR)NULL);
  ...
  SetWindowPos(graphicsWindow, HWND_TOP, graphicsRect.left, graphicsRect.top,
               RectWidth(&graphicsRect) + dx, RectHeight(&graphicsRect) + dy,
               0);
...
static LONG FAR PASCAL GraphicsEventProc(HWND hwnd, UINT msg, WPARAM wParam,
                                         LPARAM lParam) {
  switch (msg) {
  // ���¹���ʹ����double buffer, �ֶ�������
  //         ���Բ���������Ϣ��������˸
  ...
  case WM_LBUTTONDOWN:
    if (g_mouse != NULL)
      g_mouse((int)LOWORD(lParam), (int)HIWORD(lParam), LEFT_BUTTON,
              BUTTON_DOWN);
    return 0;
  ...
  case WM_MBUTTONDOWN:
    if (g_mouse != NULL)
      g_mouse((int)LOWORD(lParam), (int)HIWORD(lParam), MIDDLE_BUTTON,
              BUTTON_DOWN);
    return 0;
  ...
  case WM_RBUTTONDOWN:
    if (g_mouse != NULL)
      g_mouse((int)LOWORD(lParam), (int)HIWORD(lParam), RIGHT_BUTTON,
              BUTTON_DOWN);
    return 0;
...
static void DisplayText(double x, double y, string text) {
  ...
  TextOut(osdc, ScaleX(x), ScaleY(y) - fontTable[currentFont].ascent, text,
          strlen(text));
...
static void DisplayFont(string font, int size, int style) {
  ...
  if (fontIndex == -1) {
    newFont = CreateFont(-size, 0, 0, 0, (style & Bold) ? FW_BOLD : FW_NORMAL,
                         (style & Italic) != 0, 0, 0, 0, 0, 0, 0, 0, fontName);
    if (newFont != NULL) {
...
static int FindExistingFont(string name, int size, int style) {
  ...
  for (i = 0; i < nFonts; i++) {
    if (StringEqual(name, fontTable[i].name) && size == fontTable[i].size &&
        style == fontTable[i].style)
      return (i);
  }
  return (-1);
}
...
static void InitColors(void) {
  ...
  DefineColor("Purple", 0.7, 0.5, 1);
  DefineColor("Pink", 1, 0.5, 0.8);
  DefineColor("Light Pink", 1, 0.8, 0.9);
  DefineColor("Cyan", 0, 1, 1);
  DefineColor("Blue Cyan", 0, 0.5, 1);
}
...
static int RectWidth(RECT *rp) { return (rp->right - rp->left); }

static int RectHeight(RECT *rp) { return (rp->bottom - rp->top); }
...
static int Round(double x) { return ((int)floor(x + 0.5)); }
...
static double InchesX(int x) { return ((double)x / xResolution); }

static double InchesY(int y) { return ((double)y / yResolution); }
...
static int PixelsX(double x) { return (Round(x * xResolution + Epsilon)); }

static int PixelsY(double y) { return (Round(y * yResolution + Epsilon)); }
...
static int ScaleX(double x) { return (PixelsX(x)); }

static int ScaleY(double y) { return (PixelsY(windowHeight - y)); }
...
void registerCharEvent(CharEventCallback callback) { g_char = callback; }

void registerMouseEvent(MouseEventCallback callback) { g_mouse = callback; }

void registerTimerEvent(TimerEventCallback callback) { g_timer = callback; }

void cancelKeyboardEvent() { g_keyboard = NULL; }

void cancelCharEvent() { g_char = NULL; }

void cancelMouseEvent() { g_mouse = NULL; }

void cancelTimerEvent() { g_timer = NULL; }

void startTimer(int id, int timeinterval) {
  SetTimer(graphicsWindow, id, timeinterval, NULL);
}

void cancelTimer(int id) { KillTimer(graphicsWindow, id); }
