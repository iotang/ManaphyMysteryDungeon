...
//  最近修改：2021年5月17日 by xdq
//            修改textbox()函数，支持汉字的输入和擦除
//  最近修改：2019年5月22日
...
void setButtonColors(char *frame, char *label, char *hotFrame, char *hotLabel,
                     int fillflag) {
  if (frame)
    strcpy(gs_button_color.frame, frame);
  if (label)
    strcpy(gs_button_color.label, label);
  if (hotFrame)
    strcpy(gs_button_color.hotFrame, hotFrame);
  if (hotLabel)
    strcpy(gs_button_color.hotLabel, hotLabel);
...
void setMenuColors(char *frame, char *label, char *hotFrame, char *hotLabel,
                   int fillflag) {
  if (frame)
    strcpy(gs_menu_color.frame, frame);
  if (label)
    strcpy(gs_menu_color.label, label);
  if (hotFrame)
    strcpy(gs_menu_color.hotFrame, hotFrame);
  if (hotLabel)
    strcpy(gs_menu_color.hotLabel, hotLabel);
...
void setTextBoxColors(char *frame, char *label, char *hotFrame, char *hotLabel,
                      int fillflag) {
  if (frame)
    strcpy(gs_textbox_color.frame, frame);
  if (label)
    strcpy(gs_textbox_color.label, label);
  if (hotFrame)
    strcpy(gs_textbox_color.hotFrame, hotFrame);
  if (hotLabel)
    strcpy(gs_textbox_color.hotLabel, hotLabel);
...
int button(int id, double x, double y, double w, double h, char *label,
  ...
  if (isEnabled && notInMenu(gs_UIState.editMouseX, gs_UIState.editMouseY) &&
    ...
    timesss++;
    // printf("%d not in %f %f %f %f\n", timesss, gs_menuRect[0],
    // gs_menuRect[1],
    //       gs_menuRect[2], gs_menuRect[3]);
...
static int menuItem(int id, double x, double y, double w, double h,
  ...
  if (inBox(gs_UIState.editMouseX, gs_UIState.editMouseY, x, x + w, y, y + h)) {
    ...
    // if (gs_UIState.mousedown) {
    if ((gs_UIState.clickedItem == id || gs_UIState.clickedItem == 0) &&
  ...
  if (gs_UIState.kbdItem == id) {
    ...
    case VK_BACK:
      if (len > 0) {
        ...
        textbuf[--len] =
            0; // ASCII字符回退一个字节，汉字回退2个字节 by xdq @20210517
        textChanged = 1;
      }
      gs_UIState.keyPress = 0;
      break;
    ...
    // char
    // input：支持输入可见ASCII码字符[32,127]，和输入汉字（占2个字节且每个字节最高位都为1）by
    // xdq @20210517
    if (((gs_UIState.charInput >= 32 && gs_UIState.charInput < 127) ||
